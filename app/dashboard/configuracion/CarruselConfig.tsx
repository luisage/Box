...
function ImagenDropzone({
  ...
  return (
    <div
      ...
      className={`relative flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-xl cursor-pointer transition-colors min-h-[130px] ${
        drag
...
function CarruselModal({ imagen, onClose, onSuccess }: ModalProps) {
  const esEdicion = !!imagen

  const [nombre,        setNombre]        = useState(imagen?.nombre ?? '')
  const [estatus,       setEstatus]       = useState(imagen?.estatus ?? true)
  const [imagenFile,    setImagenFile]    = useState<File | null>(null)
  const [imagenPreview, setImagenPreview] = useState<string | null>(imagen?.url ?? null)
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = esEdicion
      ? await actualizarImagenCarrusel(fd)
      : await crearImagenCarrusel(fd)

    if (result.success) {
      onSuccess(esEdicion ? 'Imagen actualizada correctamente.' : 'Imagen agregada correctamente.')
    } else {
        ...
        <div className="flex items-start justify-between px-6 py-5 border-b border-gray-800">
          <div>
            <h2 className="text-white font-bold text-base">
              {esEdicion ? 'Editar imagen' : 'Nueva imagen de carrusel'}
            </h2>
            {esEdicion && imagen!.nombre && (
              <p className="text-gray-400 text-sm mt-0.5">{imagen!.nombre}</p>
            )}
          </div>
...
export default function CarruselConfig() {
  ...
  const [editando,   setEditando]   = useState<ImagenRow | null>(null)
  const [toast,      setToast]      = useState<string | null>(null)

  const cargar = useCallback(async () => {
    setLoading(true)
    const data = await getImagenesCarrusel()
    setImagenes(data as ImagenRow[])
    setLoading(false)
  }, [])
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-white font-bold text-lg">Imágenes del carrusel</h2>
          <p className="text-gray-500 text-sm mt-0.5">
            Administra las imágenes que se muestran en el carrusel del inicio
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-gray-500 text-sm">No hay imágenes en el carrusel</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800">
                  {['Vista previa', 'Nombre', 'URL', 'Estatus', ''].map((h, i) => (
              ...
              <tbody className="divide-y divide-gray-800/60">
                {imagenes.map((img) => (
                  <tr key={img.id} className="hover:bg-gray-800/40 transition-colors">
                    <td className="px-5 py-3">
                      <div className="relative w-20 h-12 rounded-lg overflow-hidden bg-gray-800 flex-shrink-0">
                        <Image
                          src={img.url} alt={img.nombre ?? 'carrusel'}
                          fill className="object-cover" unoptimized
                        />
                      </div>
                    </td>
                    <td className="px-5 py-3 text-gray-100 font-medium whitespace-nowrap">
                      {img.nombre ?? <span className="text-gray-600 italic text-xs">—</span>}
                    </td>
                    <td className="px-5 py-3 text-gray-500 max-w-[200px]">
                      <span className="line-clamp-1 text-xs font-mono">{img.url}</span>
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap">
                      <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${img.estatus ? 'bg-green-400' : 'bg-gray-500'}`} />
                        {img.estatus ? 'Activo' : 'Inactivo'}
                      </span>
                    </td>
                    <td className="px-5 py-3 text-right">
                      <button
                        onClick={() => setEditando(img)}
      ...
      {modalOpen && (
        <CarruselModal onClose={() => setModalOpen(false)} onSuccess={handleSuccess} />
      )}
      {editando && (
        <CarruselModal imagen={editando} onClose={() => setEditando(null)} onSuccess={handleSuccess} />
      )}
